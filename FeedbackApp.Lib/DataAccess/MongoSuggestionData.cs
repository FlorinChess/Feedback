...
public class MongoSuggestionData : ISuggestionData
{
    ...
    public async Task UpvoteSuggetion(string suggetionId, string userId)
    {
        ...
        try
        {
            ...
            var suggestion = (await suggestionsInTransaction.FindAsync(s => s.Id == suggetionId)).First();
            ...
            await suggestionsInTransaction.ReplaceOneAsync(session, s => s.Id == suggetionId, suggestion);
            ...
            if (isUpvote)
            {
                user.VotedOnSuggetions.Add(new BasicSuggestionModel(suggestion));
            }
            else
            {
                var suggestionToRemove = user.VotedOnSuggetions.Where(s => s.Id == suggetionId).First();
                user.VotedOnSuggetions.Remove(new BasicSuggestionModel(suggestion));
            }
    ...
    public async Task CreateSuggestion(SuggestionModel suggestion)
    {
        ...
        try
        {
            ...
            user.AuthoredSuggetions.Add(new BasicSuggestionModel(suggestion));
            await usersInTransaction.ReplaceOneAsync(session, u => u.Id == user.Id, user);
